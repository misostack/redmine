...
function buildFilterRow(field, operator, values) {
  ...
  var tr = $('<tr class="filter">')
    .attr("id", "tr_" + fieldId)
    .html(
      '<td class="field"><input checked="checked" id="cb_' +
        fieldId +
        '" name="f[]" value="' +
        field +
        '" type="checkbox"><label for="cb_' +
        fieldId +
        '"> ' +
        filterOptions["name"] +
        "</label></td>" +
        '<td class="operator"><select id="operators_' +
        fieldId +
        '" name="op[' +
        field +
        ']"></td>' +
  ...
  for (i = 0; i < operators.length; i++) {
    var option = $("<option>")
      .val(operators[i])
      .text(operatorLabels[operators[i]]);
  ...
  switch (filterOptions["type"]) {
    ...
    case "list_subprojects":
      tr.find("td.values").append(
        '<span style="display:none;"><select class="value" id="values_' +
          fieldId +
          '_1" name="v[' +
          field +
          '][]"></select>' +
          ' <span class="toggle-multiselect icon-only ' +
          (values.length > 1 ? "icon-toggle-minus" : "icon-toggle-plus") +
          '">&nbsp;</span></span>'
      );
    ...
    case "date_past":
      tr.find("td.values").append(
        '<span style="display:none;"><input type="date" name="v[' +
          field +
          '][]" id="values_' +
          fieldId +
          '_1" size="10" class="value date_value" /></span>' +
          ' <span style="display:none;"><input type="date" name="v[' +
          field +
          '][]" id="values_' +
          fieldId +
          '_2" size="10" class="value date_value" /></span>' +
          ' <span style="display:none;"><input type="text" name="v[' +
          field +
          '][]" id="values_' +
          fieldId +
          '" size="3" class="value" /> ' +
    ...
    case "relation":
      tr.find("td.values").append(
        '<span style="display:none;"><input type="text" name="v[' +
          ...
          '][]" id="values_' +
          fieldId +
          '" size="6" class="value" /></span>' +
          '<span style="display:none;"><select class="value" name="v[' +
          field +
          '][]" id="values_' +
          fieldId +
          '_1"></select></span>'
    ...
    case "tree":
      tr.find("td.values").append(
        '<span style="display:none;"><input type="text" name="v[' +
          ...
          '][]" id="values_' +
          fieldId +
          '_1" size="14" class="value" /></span>' +
          ' <span style="display:none;"><input type="text" name="v[' +
          field +
          '][]" id="values_' +
          fieldId +
          '_2" size="14" class="value" /></span>'
      );
...
function toggleMultiSelect(el) {
  ...
  if (el.attr("multiple")) {
    ...
    if (isWorkflow) {
      el.find("option[value=all]").show();
    }
    el.attr("size", 1);
  } else {
    el.attr("multiple", true);
    if (isWorkflow) {
      el.find("option[value=all]").attr("selected", false).hide();
    }
    if (el.children().length > 10) el.attr("size", 10);
    else el.attr("size", 4);
  }
}
...
function displayTabsButtons() {
  ...
  $("div.tabs").each(function () {
    ...
    if (
      tabsWidth < el.width() - bw &&
      (lis.length === 0 || lis.first().is(":visible"))
    ) {
      el.find("div.tabs-buttons").hide();
    } else {
      el.find("div.tabs-buttons")
        .show()
        .children("button.tab-left")
        .toggleClass("disabled", numHidden == 0);
    }
  });
}
...
function updateIssueFrom(url, el) {
  $(
    "#all_attributes input, #all_attributes textarea, #all_attributes select"
  ).each(function () {
...
$(document).ready(function () {
  ...
  observeSearchfield(
    ...
    null,
    $("#projects-quick-search").data("automcomplete-url")
  );
...
function toggleMultiSelectIconInit() {
  $(
    ".toggle-multiselect:not(.icon-toggle-minus), .toggle-multiselect:not(.icon-toggle-plus)"
  ).each(function () {
...
$(document).ready(function () {
  $("#content").on(
    "change",
    "input[data-disables], input[data-enables], input[data-shows]",
    toggleDisabledOnChange
  );
  toggleDisabledInit();
...
function setFilecontentContainerHeight() {
  ...
  if (
    ...
    $filecontainer.find(fileTypeSelectors.join(",")).length === 1
  ) {
...
function inlineAutoComplete(element) {
  ...
  if (element.dataset.tribute === "true") {
    return;
  }
...
function highlightStatus() {
  $("td.status:contains('Inprogress')").css(
    "background-color",
    "#ffc107!important"
  );
